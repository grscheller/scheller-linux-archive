...
main = do
  ...
  case args of
    ...
    [ "-f", start, end] -> printTriples $ pythagTriplesFast (read start) (read end)
    ["-fs", start, end] -> printTriples $ sort $ map sortTriple $ pythagTriplesFast (read start) (read end)
    ["-o1", end] -> printTriples $ pythagTriplesOrdered1 3 (read end)
    ["-o2", end] -> printTriples $ pythagTriplesOrdered2 4 (read end)
    [ "-f", end] -> printTriples $ pythagTriplesFast     2 (read end)
    ["-fs", end] -> printTriples $ sort $ map sortTriple $ pythagTriplesFast 2 (read end)
