// Minimal changes needed to compile with scalaz.concurrent.Actor
//
//   Hangs in the same way as in fpinscala, but I really
//   don't know what I am doing.  My guess is that I need
//   get rid of all references to es and have the library
//   come up with a default one via implicit conversions.
//
//   Also, only have Actors handle the parallelism.
//
//   I need to find something where error handling with 
//   scalaz Actors works and see if I can figure out what
//   is different with the fpinscala implimentation.
//
//   My metathinking - if error handling in scalaz.concurrent.Actor
//   broken, it would have been fixed by now.
//
      ...
      def apply(es: ExecutorService) =
        new ParFuture[C] {
          def apply(cb: C => Unit, onError: Throwable => Unit): Unit = {
            ...
            val combiner = Actor.actor[Either[A,B]]( {
