...
If `A` and `B` be collection of words,
```
   A ∪ B = { w ∥ w ∈ A or w ∈ b }
   A⋅B = { ab ∥ a ∈ A and b ∈ B }
```
...
a formal language which can be expressed using a _regular exprssion_.
...
* `^`: match beginning of string, if at start of RE, else match itself
* `$`: match end of string, if at end of RE, else match itself
...
* `[ ]`: match any char enclosed between brackets
...
sense to use BRE in vim.  Probably more likely done for backward
